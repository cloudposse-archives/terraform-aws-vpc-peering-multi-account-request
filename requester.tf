...
resource "aws_route" "requester" {
  count                     = "${local.enabled ? local.requester_aws_route_table_ids_count * length(var.accepter_cidr_block_associations) : 0}"
  route_table_id            = "${element(local.requester_aws_route_table_ids, ceil(count.index/length(var.accepter_cidr_block_associations)))}"
  destination_cidr_block    = "${lookup(var.accepter_cidr_block_associations[count.index % length(var.accepter_cidr_block_associations)], "cidr_block")}"
  vpc_peering_connection_id = "${join("", aws_vpc_peering_connection.requester.*.id)}"
